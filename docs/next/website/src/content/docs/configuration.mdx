...
Privacy mode is a camera-safe redaction layer for live Herdr rendering. It masks configured private terms anywhere they appear in the rendered UI, including panes, tabs, sidebar labels, toasts, and overlays. Toggle it with the configured `toggle_privacy_mode` keybinding; the sidebar footer shows the current lock/unlock state.

```toml
[privacy]
enabled = false
patterns_file = "~/.config/herdr/privacy-patterns.txt"
patterns = ["Private Client Name", "private-client.example"]
replacement = "█"
...
The patterns file is newline-delimited. Blank lines and `#` comments are ignored. Missing pattern files are ignored so you can keep a private, machine-local list outside your repo.

Privacy mode only redacts what Herdr renders as terminal text. It does not inspect browser windows, desktop notifications, images, audio, or content already captured by a recorder before privacy mode was turned on.
